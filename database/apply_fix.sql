-- CONSOLIDATED FIX FOR ORDER CREATION RLS (v3 - Signature Fix)
-- Run this in your Supabase SQL Editor

-- 1. Ensure the delivery_method column exists in the orders table
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS delivery_method TEXT;

-- 2. Ensure Unique Index for Idempotency
CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_idempotency_key 
ON public.orders(idempotency_key) 
WHERE idempotency_key IS NOT NULL;

-- 3. DROP old versions to avoid "function not unique" errors
-- We drop both the 9-parameter and 10-parameter versions to start fresh
...
-- 4. Create the final RPC with SECURITY DEFINER
...
-- 5. Create reserve_stock with SECURITY DEFINER
...
BEGIN
  IF p_country = 'germany' THEN
    ...
    WHERE id = p_product_id AND stock_germany >= p_quantity;
  ELSE
    UPDATE public.products 
    SET stock_denmark = stock_denmark - p_quantity 
    WHERE id = p_product_id AND stock_denmark >= p_quantity;
  END IF;
...
-- 6. Explicitly grant EXECUTE permissions using full signatures to avoid ambiguity
...
REVOKE EXECUTE ON FUNCTION public.reserve_stock(uuid, text, integer) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.reserve_stock(uuid, text, integer) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.reserve_stock(uuid, text, integer) TO service_role;
